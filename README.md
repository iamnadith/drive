...
The build applies the idempotent `supabase/drive_schema.sql` before Next.js compiles. Configure `POSTGRES_URL` for the pooled runtime connection and provide `POSTGRES_URL_NON_POOLING` when schema preparation needs a direct connection. PostgreSQL TLS is enabled by default; set `POSTGRES_SSL=false` only when your database explicitly requires plaintext connections. The runtime pool defaults to two connections per server process (`POSTGRES_POOL_MAX` can be set to a measured provider-appropriate value). The build fails before deployment if the schema cannot be prepared, preventing first-request schema races.
