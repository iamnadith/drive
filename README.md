...
The build applies the idempotent `supabase/drive_schema.sql` before Next.js compiles. Configure `POSTGRES_URL` as the single PostgreSQL connection string used by the dashboard, schema preparation, and Workers. PostgreSQL TLS is enabled by default; set `POSTGRES_SSL=false` or `DISABLE_POSTGRES_SSL=true` only when your database explicitly requires plaintext connections. Both flags are honored by the runtime pool and build-time schema preparation. The runtime pool defaults to two connections per server process (`POSTGRES_POOL_MAX` can be set to a measured provider-appropriate value). The build fails before deployment if the schema cannot be prepared, preventing first-request schema races.
...
First-run setup verifies the configured PostgreSQL connection, Resend email credentials, and public application URL before allowing Worker or administrator setup. PostgreSQL TLS is enabled by default; `POSTGRES_SSL=false` or `DISABLE_POSTGRES_SSL=true` is only needed for databases that explicitly require plaintext transport. No Supabase URL, publishable key, or service-role key is required for database access. Google OAuth (`GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET`) and Text.lk SMS (`TEXTLK_API_TOKEN`) are optional; their controls are omitted when unavailable. Healthy Cloudflare scripts are adopted or reused, missing queue consumers and Worker cron triggers are repaired in place, and only missing or unhealthy Worker scripts are uploaded again. Established pages render immediately and do not poll infrastructure; a failed Worker-backed request triggers reconciliation and repair.
