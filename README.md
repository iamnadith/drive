...
First-run setup verifies PostgreSQL connectivity, Supabase URL/public/server credentials, Resend email credentials, the public application URL, and a 16+ character `CRON_SECRET` before allowing Worker or administrator setup. Google OAuth (`GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET`) and Text.lk SMS (`TEXTLK_API_TOKEN`) are optional; their controls are omitted when unavailable. Healthy Cloudflare scripts are adopted or reused, missing queue consumers and cron triggers are repaired in place, and only missing or unhealthy Worker scripts are uploaded again. The Backend Orchestrator requests reconciliation during its normal cycle, while the protected Vercel cron provides a daily recovery path if that Backend Worker is itself unavailable.
