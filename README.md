...
The build applies the idempotent `supabase/drive_schema.sql` before Next.js compiles. Configure `POSTGRES_URL` as the single PostgreSQL connection string used by the dashboard, schema preparation, and Workers. PostgreSQL TLS is enabled by default. For a PostgreSQL server that requires plaintext transport, add `sslmode=disable` to the `POSTGRES_URL` query string; this mode is honored by the runtime pool, schema preparation, and all Workers, so no separate SSL environment variable is needed. The older `POSTGRES_SSL` and `DISABLE_POSTGRES_SSL` flags remain optional overrides. The runtime pool defaults to two connections per server process (`POSTGRES_POOL_MAX` can be set to a measured provider-appropriate value). The build fails before deployment if the schema cannot be prepared, preventing first-request schema races.
...
The **Workers** page deploys Backend Orchestrator, File Scanner, and Migration Orchestrator from immutable release bundles. A superadmin can use one account-scoped Cloudflare token for every Worker or a separate token for each role. The installer creates the required Queues, injects PostgreSQL and role secrets, deploys in dependency order, saves Worker URLs atomically, verifies authenticated status, and enables the Workers only after every check succeeds.

By default the panel reads `https://github.com/iamnadith/Drive/releases/latest/download/manifest.json`. Override the repository with `GITHUB_WORKER_SOURCE_REPO` or the full location with `CLOUDFLARE_WORKER_MANIFEST_URL`. Every push to `main` publishes a checksum-protected release named from the commit SHA; `workers-v*` tags and manual runs are also supported. Cloudflare tokens are encrypted with AES-256-GCM before database storage and may be reused, revealed on demand by a superadmin, or replaced. A dedicated `CLOUDFLARE_TOKEN_ENCRYPTION_KEY` is optional; when no authentication secret is configured, a domain-separated key is derived from the server-only PostgreSQL connection configuration, so no additional encryption-key environment variable is required. The Workers page provides separate controls to check for a newer release and to force-redeploy all three bundles. Failed installations retain a durable checkpoint and can be resumed or restarted explicitly.

First-run setup verifies the configured PostgreSQL connection, Resend email credentials, and public application URL before allowing Worker or administrator setup. PostgreSQL TLS is enabled by default; when plaintext is required, set `sslmode=disable` on `POSTGRES_URL` instead of adding a separate SSL environment variable. No Supabase URL or publishable key is used for database access. The service-role key is retained only as a legacy decryption input for existing Cloudflare tokens until those saved tokens have been safely rotated. Google OAuth (`GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET`) and Text.lk SMS (`TEXTLK_API_TOKEN`) are optional; their controls are omitted when unavailable. Healthy Cloudflare scripts are adopted or reused, missing queue consumers and Worker cron triggers are repaired in place, and only missing or unhealthy Worker scripts are uploaded again. Established pages render immediately and do not poll infrastructure; a failed Worker-backed request triggers reconciliation and repair.
