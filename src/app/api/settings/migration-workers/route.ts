...
export async function PUT(request: Request) {
  ...
  if (!auth.ok) return auth.response
  try {
    const body = await request.json().catch(() => ({})) as { serverUrl?: unknown; sharedSecret?: unknown }
    const settings = await saveMigrationWorkerSettings({ serverUrl: body.serverUrl, sharedSecret: body.sharedSecret })
