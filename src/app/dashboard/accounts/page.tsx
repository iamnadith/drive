...
export default function AccountsPage() {
  ...
  const [viewTotpSecondsLeft, setViewTotpSecondsLeft] = React.useState<
    number | null
  >(null);
  const [totpCode, setTotpCode] = React.useState<string | null>(null);
  const [totpSecondsLeft, setTotpSecondsLeft] = React.useState<number | null>(
    null,
  );
  const [editTotpCode, setEditTotpCode] = React.useState<string | null>(null);
  const [editTotpSecondsLeft, setEditTotpSecondsLeft] = React.useState<
    number | null
  >(null);
  ...
  React.useEffect(() => {
    ...
    const tick = async () => {
      ...
      try {
        const { otp, secondsRemaining } =
          await generateTotp(editTwoFactorSecret);
        setEditTotpCode(otp);
  ...
  const handleOpenMigrationCleanup = async (account: Account) => {
    try {
      ...
      if (!res.ok)
        throw new Error(data.error || "Unable to load linked migrations");

      const rows: AccountMigrationRecord[] = (data.migrations ?? []).map(
        (m: any) => ({
          id: String(m.id ?? ""),
  ...
  const handleDelete = async (
    id: string,
    opts?: { silentBlockedError?: boolean },
  ) => {
    try {
  ...
  const handleDeleteLinkedMigrationsAndAccount = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/accounts/${migrationCleanupAccount.id}/migrations`,
        {
          method: "DELETE",
        },
      );
      const data = await res.json().catch(() => ({}));
      if (!res.ok)
        throw new Error(data.error || "Unable to delete linked migrations");
        ...
        prev.map((acc) =>
          acc.id === updated.id
            ? {
                ...
                twoFactorSecret: updated.twoFactorSecret ?? editTwoFactorSecret,
    ...
    <DashboardPage>
            ...
            <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
              ...
              <DialogContent className="max-h-[88vh] sm:max-h-[94vh] sm:max-w-3xl flex flex-col rounded-2xl">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Save Cloudflare credentials (email, password, 2FA, API
                    token, R2 keys) to use for migrations and management.
                  </DialogDescription>
                </DialogHeader>
                    ...
                    <div className="rounded-lg border bg-muted/40 p-4 space-y-4">
                      ...
                      <div className="grid gap-4 md:grid-cols-2">
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  try {
                                    ...
                                    toast.error(
                                      "Unable to copy R2 Access Key ID",
                                    );
                                  }
                                }}
                                className="inline-flex items-center gap-1 rounded-md border px-1.5 py-0.5 text-muted-foreground hover:bg-muted"
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  try {
                                    ...
                                    toast.error(
                                      "Unable to paste R2 Access Key ID",
                                    );
                                  }
                                }}
                                className="inline-flex items-center gap-1 rounded-md border px-1.5 py-0.5 text-muted-foreground hover:bg-muted"
                        ...
                        <div className="space-y-2 md:col-span-2">
                          <div className="flex items-center justify-between gap-2">
                            <Label htmlFor="r2SecretAccessKey">
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  try {
                                    ...
                                    toast.success(
                                      "R2 Secret Access Key copied",
                                    );
                                  } catch {
                                    toast.error(
                                      "Unable to copy R2 Secret Access Key",
                                    );
                                  }
                                }}
                                className="inline-flex items-center gap-1 rounded-md border px-1.5 py-0.5 text-muted-foreground hover:bg-muted"
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  try {
                                    ...
                                    toast.error(
                                      "Unable to paste R2 Secret Access Key",
                                    );
                                  }
                                }}
                                className="inline-flex items-center gap-1 rounded-md border px-1.5 py-0.5 text-muted-foreground hover:bg-muted"
              ...
              <div className="space-y-6 pb-6">
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        value={viewTotpCode ?? ""}
                        placeholder={
                          viewAccount.twoFactorSecret ? "Calculating..." : ""
                        }
                        readOnly
                        disabled
                      />
                ...
                <div className="rounded-lg border bg-muted/40 p-4 pb-6 space-y-4">
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between gap-2">
                        <Label htmlFor="view-r2AccessKeyId">
                          R2 Access Key ID
                        </Label>
                        <div className="flex gap-1 text-[11px]">
                          <button
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between gap-2">
                        <Label htmlFor="view-r2SecretAccessKey">
                          R2 Secret Access Key
                        </Label>
                        <div className="flex gap-1 text-[11px]">
                          <button
      ...
      {settingsAccount && (
        <Dialog
          ...
          onOpenChange={() => setSettingsAccount(null)}
        >
          <DialogContent
            showCloseButton={false}
            className="max-h-[88vh] sm:max-h-[94vh] sm:max-w-3xl flex flex-col rounded-2xl"
              ...
              <div className="space-y-6 pb-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        <Input
                          ...
                          onChange={(e) =>
                            setEditTwoFactorSecret(e.target.value)
                          }
                          required
                          className="pr-8"
                        />
                ...
                <div className="rounded-lg border bg-muted/40 p-4 pb-6 space-y-4">
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between gap-2">
                        <Label htmlFor="edit-r2AccessKeyId">
                          R2 Access Key ID
                        </Label>
                        <div className="flex gap-1 text-[11px]">
                          <button
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between gap-2">
                        <Label htmlFor="edit-r2SecretAccessKey">
                          R2 Secret Access Key
                        </Label>
                        <div className="flex gap-1 text-[11px]">
                          <button
                      ...
                      <div className="relative">
                        <Input
                          ...
                          onChange={(e) =>
                            setEditR2SecretAccessKey(e.target.value)
                          }
                          required
                          className="pr-8"
                        />
          ...
          <DialogContent className="sm:max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                <span className="font-semibold">
                  {migrationCleanupAccount.name ||
                    migrationCleanupAccount.email}
                </span>{" "}
                is referenced by migrations. Delete these records, then remove
                the account.
              </DialogDescription>
            </DialogHeader>
                        ...
                        <TableRow key={migration.id}>
                          ...
                          <TableCell className="text-right">
                            <Button
                              ...
                              onClick={() =>
                                handleDeleteMigrationRecord(migration.id)
                              }
                            >
                              {deletingMigrationIds[migration.id]
                                ? "Deleting..."
            ...
            <DialogFooter>
              ...
              <Button
                ...
                variant="destructive"
                disabled={
                  isLoadingLinkedMigrations || isDeletingLinkedMigrations
                }
