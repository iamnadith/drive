...
function CardHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "@container/card-header grid min-w-0 auto-rows-min grid-rows-[auto_auto] items-start gap-2 px-4 pt-2 sm:px-5 sm:pt-4 md:px-4 md:pt-3 sm:has-data-[slot=card-action]:grid-cols-[minmax(0,1fr)_auto] [.border-b]:pb-4 sm:[.border-b]:pb-5 md:[.border-b]:pb-4",
        className
      )}
      {...props}
    />
...
function CardContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("min-w-0 px-4 pb-2 sm:px-5 sm:pb-4 md:px-4 md:pb-3", className)}
      {...props}
    />
  )
}
