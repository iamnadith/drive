...
export async function getSystemReadiness() {
  ...
  const requirements: RequirementStatus[] = [
    ...
    { id: "cron", label: "Recovery scheduler secret", description: "Protects unattended daily recovery when the Backend Worker itself is unavailable.", configured: String(process.env.CRON_SECRET || "").trim().length >= 16, variables: ["CRON_SECRET (16+ random characters)"] },
