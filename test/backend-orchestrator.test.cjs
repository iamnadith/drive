...
test("backend orchestrator uses the authenticated panel configuration handshake", () => {
  ...
  assert.match(orchestrator, /BACKEND_ORCHESTRATOR_SECRET: string/)
  assert.match(orchestrator, /POSTGRES_URL: string/)
  assert.match(orchestrator, /normalized === "1" \|\| normalized === "true"/)
  assert.match(orchestrator, /disablePostgresSsl \|\| sslMode === "disable" \? false : \{ rejectUnauthorized: false \}/)
  assert.doesNotMatch(orchestrator, /isSupabase|supabase\.com/)
  ...
  assert.match(deploy, /new URL\(String\(value\)\.trim\(\)\)\.toString\(\)/)
  assert.doesNotMatch(deploy, /url\.port = "5432"/)
  ...
  assert.match(configRoute, /postgresSslDisabled\(postgresUrl\)/)
  assert.match(configRoute, /postgresSslDisabled\(postgresUrl\)/)
  assert.match(deploy, /DISABLE_POSTGRES_SSL: config\.disablePostgresSsl === true \? "1" : "0"/)
  assert.match(settings, /secretConfigured:/)
  assert.match(settings, /sharedSecret: settings\.sharedSecret/)
  assert.match(settingsPage, /admin-only page displays the saved secret/)
...
test("worker sync is aggregate-only and resumable across CPU-limited invocations", () => {
  ...
  // Free Workers allows only a small number of concurrent outgoing
  ...
  const selectNextAccount = orchestrator.slice(orchestrator.indexOf("async function selectNextAccount"), orchestrator.indexOf("async function loadSyncProgress"))
  const selectAccountById = orchestrator.slice(orchestrator.indexOf("async function selectAccountById"), orchestrator.indexOf("async function saveSyncProgress"))
  assert.match(selectNextAccount, /where api_token<>''\s+and status='active'/)
  assert.match(selectAccountById, /where id=\$1 and api_token<>'' and status='active' limit 1/)
  assert.match(orchestrator, /drive_backend_orchestrator_progress/)
  assert.match(orchestrator, /async function legacyEnsureProgressSchema/)
  assert.match(orchestrator, /await ensureSchema\(db\)/)
  assert.doesNotMatch(orchestrator, /await legacyEnsureProgressSchema\(db\)/)
  assert.match(orchestrator, /const REQUIRED_SCHEMA_VERSION = 2026091303/)
  assert.match(orchestrator, /select version from drive_schema_meta where id=true limit 1/)
  assert.match(orchestrator, /deploy the panel schema first/)
  ...
  assert.match(orchestrator, /R2 analytics missing for one or more buckets/)
  ...
  assert.match(orchestrator, /bucket_fingerprint/)
  ...
  assert.match(orchestrator, /pendingMetricDecreases\(db, account\.id, metrics\)/)
  ...
  assert.match(orchestrator, /select account_id, bucket_names, bucket_offset, reconciled, metrics_incomplete, pending_decreases from drive_backend_orchestrator_progress/)
  assert.match(orchestrator, /Persist the account and bucket list before doing reconciliation/)
  assert.match(orchestrator, /bucketOffset: nextOffset,\s+reconciled: true/)
...
test("worker packages and GitHub workflow use their permanent names", () => {
  ...
  assert.match(wrangler, /enable_ctx_exports/)
