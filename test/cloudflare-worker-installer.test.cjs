...
test("installer deploys in dependency order and saves configuration before enabling", () => {
  ...
  const synchronize = installer.indexOf("await syncAllGitHubWorkerSecrets({", schedules)
...
test("installer is resumable, locked and keeps secrets on release redeploy", () => {
  ...
  assert.match(installer, /githubSecretsSynchronized[\s\S]*previousRuntime\.migration\.orchestratorUrl[\s\S]*syncAllGitHubWorkerSecrets/)
