...
test("bucket history writes only changes and permanent deletion tombstones", () => {
  ...
  assert.match(schema, /create table if not exists drive_storage_stats_history/)
  assert.match(schema, /alter table public\.drive_bucket_stat_history rename to drive_storage_stats_history/)
