...
test('migration pool abort and job deletion preserve orchestrator ownership and stopping guards', () => {
  ...
  assert.match(jobPage, /onClick=\{\(\) => setDeleteOpen\(true\)\}/)
  assert.match(jobPage, /method: "DELETE"/)
  assert.match(jobPage, /Migration Orchestrator must confirm all workers[\s\S]*?cannot be deleted until/)
