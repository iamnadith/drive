...
test('canceled migration and bucket states cannot be masked by stale verifying snapshots', () => {
  ...
  assert.match(cancelRoute, /async function abortGitHubDispatchesForMigration\(migrationId: string\)/)
  assert.match(cancelRoute, /r\.payload->>'migrationId'=\$1[\s\S]*?a\.provider='github_actions'/)
  assert.match(cancelAction, /cancelDispatchResult = workerMode \? await abortGitHubDispatchesForMigration\(id\)/)
  ...
  assert.match(cancelRoute, /cancellation\.status === "canceled"[\s\S]*?updateAgentRun\(run\.id,[\s\S]*?status: "canceled"/)
