...
test('active worker migrations cannot be frozen by historical read-only markers', () => {
  ...
  assert.match(detailsRoute, /executionMode === "migration_workers" \? listMigrationWorkerRuns/)
...
test('Super Slurper repair re-enters the shared worker-pool scan, queue, copy, and verification lifecycle', () => {
  ...
  assert.match(action, /wakeMigrationService\("orchestrator"\)/)
...
test('worker-pool repair reconciles destinations before copy and bucket preparation stays worker-owned', () => {
  ...
  const workerStart = startRoute.match(/if \(migration\.options\.executionMode === "migration_workers"\) \{([\s\S]*?)\n    const startedAt/)

  assert.ok(workerStart, 'worker-pool start branch should remain a small orchestration request')
  assert.doesNotMatch(workerStart[1], /r2ListBuckets|r2CreateBucketViaApi|ensureTargetBucketExists/)
