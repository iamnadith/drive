...
test("users page is server-paginated with safe columns and analytics reads only user aggregates", () => {
  ...
  const analytics = read("src/app/api/dashboard/analytics/route.ts")
  assert.match(analytics, /getAnalyticsCatalogSummary/)
  ...
  const catalogStart = analytics.indexOf("async function getAnalyticsCatalogSummary()")
  const catalogEnd = analytics.indexOf("async function getAnalyticsSqlSummary()", catalogStart)
  const repairProjection = analytics.slice(catalogStart, catalogEnd)
...
test("analytics aggregates bucket and verification summaries in one database round trip", () => {
  ...
  const catalogStart = analytics.indexOf("async function getAnalyticsCatalogSummary()")
  const catalogEnd = analytics.indexOf("async function getAnalyticsSqlSummary()", catalogStart)
  assert.ok(catalogStart >= 0 && catalogEnd > catalogStart, "expected a compact shared analytics catalog query")
  const catalog = analytics.slice(catalogStart, catalogEnd)
  assert.equal((catalog.match(/queryDb</g) ?? []).length, 1)
  assert.match(catalog, /from public\.drive_users/i)
  assert.match(catalog, /from public\.drive_migrations[\s\S]*?order by created_at desc,id desc\s+limit 100/i)
  assert.match(catalog, /from public\.drive_repair_jobs[\s\S]*?order by created_at desc,id desc\s+limit 100/i)
  ...
  const analyticsReads = analytics.slice(analytics.indexOf("const [\n    catalog"), analytics.indexOf("const { migrations, users, repairJobs } = catalog"))
